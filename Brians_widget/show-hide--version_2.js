...
$(document).ready(function () {
   ...
   function loadingFade() {
      ...
      // put the strikethroughs collection out here where it won't be overwritten by each
      strikethroughs = {};
      sortedChanges.each(function (index) {
         span = $(this);
         strikethroughs[index] = span.hasClass("overstrike");
         // if there is an overstrike, remove class and add back in after the fadeIn
         if (strikethroughs[index]) {
            span.removeClass("overstrike");
         }
         span.delay((index) * 2500).fadeIn(2500).delay(2500).queue(function() {
            if (strikethroughs[index]) { 
               $(this).addClass("overstrike"); 
            }
         });
   ...
   function elementToggler(span) {
      ...
      if (spanId) {
         ...
         } else if (span.attr("title") && span.attr("title").indexOf("add insertion") !== -1) {
            aboveInsertion(span);
         } else {
